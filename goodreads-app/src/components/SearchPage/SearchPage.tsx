...
import { Book, State, Props, DispatchProps, Result } from "../../types/types";
...
class Search extends React.Component<Props, State> {
  ...
  renderSuggestions = () => {
    ...
    const { items = [] } =
      R.find((book: Result) => book.query == query, books) || {};
    // let sortedBooks = items
    //   .sort((a: Book, b: Book) => b.avgRating - a.avgRating)
    //   .slice(0, 5);
    let sortedBooks = R.pipe(
      R.sort((a: Book, b: Book) => b.avgRating - a.avgRating),
      R.slice(0, 5)
    )(items);
    if (userInput.length === 0 || userInput !== query) {
      // if (
      //   R.either(R.equals(userInput.length, 0), R.not(R.equals(userInput, query)))
      // ) {
